<?php
/**
 * Global Easy Job Portal functions.
 *
...
if ( ! function_exists( 'get_job_listings' ) ) :
	...
	function get_job_listings( $args = [] ) {
		...
		} elseif ( 0 === intval( get_option( 'easy_job_portal_hide_expired', get_option( 'easy_job_portal_hide_expired_content', 1 ) ) ) ) {
		...
		if ( ! is_null( $args['filled'] ) || 1 === absint( get_option( 'easy_job_portal_hide_filled_positions' ) ) ) {
			$query_args['meta_query'][] = [
				'key'     => '_filled',
				'value'   => '1',
				'compare' => $args['filled'] ? '=' : '!=',
		...
		if ( ! empty( $args['search_categories'] ) ) {
			...
			$operator                  = 'all' === get_option( 'easy_job_portal_category_filter_type', 'all' ) && count( $args['search_categories'] ) > 1 ? 'AND' : 'IN';
			$query_args['tax_query'][] = [
		...
		$easy_job_portal_keyword = sanitize_text_field( $args['search_keywords'] );

		if ( ! empty( $easy_job_portal_keyword ) && strlen( $easy_job_portal_keyword ) >= apply_filters( 'easy_job_portal_get_listings_keyword_length_threshold', 2 ) ) {
			$query_args['s'] = $easy_job_portal_keyword;
			add_filter( 'posts_search', 'get_job_listings_keyword_search' );
		}

		$query_args = apply_filters( 'easy_job_portal_get_listings', $query_args, $args );
		...
		/** This filter is documented in easy-job-portal.php */
		...
		if ( apply_filters( 'get_job_listings_cache_results', true ) ) {
			...
			$query_args_hash      = 'jm_' . md5( $to_hash . EASY_JOB_VERSION ) . Easy_Job_Portal_Cache_Helper::get_transient_version( 'get_job_listings' );
			$result               = false;
...
if ( ! function_exists( 'get_job_listings_keyword_search' ) ) :
	...
	function get_job_listings_keyword_search( $search ) {
		global $wpdb, $easy_job_portal_keyword;
		...
		if ( apply_filters( 'job_listing_search_post_meta', true ) ) {
			...
			if ( $searchable_meta_keys ) {
				$conditions[] = "{$wpdb->posts}.ID IN ( SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key IN ( '" . implode( "','", array_map( 'esc_sql', $searchable_meta_keys ) ) . "' ) AND meta_value LIKE '%" . esc_sql( $easy_job_portal_keyword ) . "%' )";
			} else {
				// No meta keys defined, search all post meta value.
				$conditions[] = "{$wpdb->posts}.ID IN ( SELECT post_id FROM {$wpdb->postmeta} WHERE meta_value LIKE '%" . esc_sql( $easy_job_portal_keyword ) . "%' )";
			}
		}

		// Search taxonomy.
		$conditions[] = "{$wpdb->posts}.ID IN ( SELECT object_id FROM {$wpdb->term_relationships} AS tr LEFT JOIN {$wpdb->term_taxonomy} AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id LEFT JOIN {$wpdb->terms} AS t ON tt.term_id = t.term_id WHERE t.name LIKE '%" . esc_sql( $easy_job_portal_keyword ) . "%' )";
		 ...
		 * @param string $easy_job_portal_keyword Search query.
		 */
		$conditions = apply_filters( 'job_listing_search_conditions', $conditions, $easy_job_portal_keyword );
		if ( empty( $conditions ) ) {
			return $search;
		}
...
if ( ! function_exists( 'get_job_listing_post_statuses' ) ) :
	...
	function get_job_listing_post_statuses() {
		return apply_filters(
			...
			[
				'draft'           => _x( 'Draft', 'post status', 'easy-job-portal' ),
				'expired'         => _x( 'Expired', 'post status', 'easy-job-portal' ),
				'preview'         => _x( 'Preview', 'post status', 'easy-job-portal' ),
				'pending'         => _x( 'Pending approval', 'post status', 'easy-job-portal' ),
				'pending_payment' => _x( 'Pending payment', 'post status', 'easy-job-portal' ),
				'publish'         => _x( 'Active', 'post status', 'easy-job-portal' ),
			]
		);
	}
endif;
...
if ( ! function_exists( 'easy_job_portal_get_filtered_links' ) ) :
	/**
	 ...
	 * @return string
	 */
	function easy_job_portal_get_filtered_links( $args = [] ) {
		$job_categories = [];
		...
		$links = apply_filters(
			'easy_job_portal_job_filters_showing_jobs_links',
			[
				'reset'    => [
					'name' => __( 'Reset', 'easy-job-portal' ),
					'url'  => '#',
				],
				'rss_link' => [
					'name' => __( 'RSS', 'easy-job-portal' ),
					'url'  => get_job_listing_rss_link(
						apply_filters(
							'easy_job_portal_get_listings_custom_filter_rss_args',
							[
		...
		if (
			...
			&& ! apply_filters( 'easy_job_portal_get_listings_custom_filter', false )
		) {
			unset( $links['reset'] );
		}
...
if ( ! function_exists( 'get_job_listing_rss_link' ) ) :
	...
	function get_job_listing_rss_link( $args = [] ) {
		$rss_link = add_query_arg( urlencode_deep( array_merge( [ 'feed' => Easy_Job_Portal_Post_Types::get_job_feed_name() ], $args ) ), home_url() );
		return $rss_link;
	}
endif;

if ( ! function_exists( 'wp_easy_job_portal_notify_new_user' ) ) :
	/**
	 ...
	 */
	function wp_easy_job_portal_notify_new_user( $user_id, $password ) {
		global $wp_version;
...
if ( ! function_exists( 'wp_easy_job_portal_create_account' ) ) :
	/**
	 ...
	 */
	function wp_easy_job_portal_create_account( $args, $deprecated = '' ) {
		...
		if ( empty( $email ) ) {
			return new WP_Error( 'validation-error', __( 'Invalid email address.', 'easy-job-portal' ) );
		}
		...
		if ( ! is_email( $email ) ) {
			return new WP_Error( 'validation-error', __( 'Your email address isn&#8217;t correct.', 'easy-job-portal' ) );
		}

		if ( email_exists( $email ) ) {
			return new WP_Error( 'validation-error', __( 'This email is already registered, please choose another one.', 'easy-job-portal' ) );
		}
		...
		$reg_errors = apply_filters( 'easy_job_portal_registration_errors', $reg_errors, $username, $email );

		do_action( 'easy_job_portal_register_post', $username, $email, $reg_errors );
		...
		$user_id = wp_insert_user( apply_filters( 'easy_job_portal_create_account_data', $new_user ) );
 ...
 * @return bool
 */
function easy_job_portal_user_can_upload_file_via_ajax() {
	$can_upload = is_user_logged_in() && easy_job_portal_user_can_post_job();

	if ( has_filter( 'easy_job_portal_ajax_file_upload_enabled' ) ) {
		_deprecated_hook( 'easy_job_portal_ajax_file_upload_enabled', '1.30.0', 'easy_job_portal_user_can_upload_file_via_ajax' );
		$can_upload = apply_filters( 'easy_job_portal_ajax_file_upload_enabled', $can_upload );
	}
	 ...
	 */
	return apply_filters( 'easy_job_portal_user_can_upload_file_via_ajax', $can_upload );
}
 ...
 * @return bool
 */
function easy_job_portal_user_can_post_job() {
	$can_post = true;

	if ( ! is_user_logged_in() ) {
		if ( easy_job_portal_user_requires_account() && ! easy_job_portal_enable_registration() ) {
			$can_post = false;
		}
	}

	return apply_filters( 'easy_job_portal_user_can_post_job', $can_post );
}
 ...
 * @return bool
 */
function easy_job_portal_user_can_edit_job( $job_id ) {
	$can_edit = true;
	...
	return apply_filters( 'easy_job_portal_user_can_edit_job', $can_edit, $job_id );
}

/**
 * Checks if the visitor is currently on a DLJP page, job listing, or taxonomy.
 *
 * @since 1.30.0
 *
 * @return bool
...
/**
 * Checks if the visitor is currently on a DLJP page.
 *
 * @since 1.30.0
 *
 * @return bool
...
function is_wpjm_page() {
	...
	if ( ! $is_wpjm_page ) {
		$wpjm_page_ids = array_filter(
			[
				get_option( 'easy_job_portal_submit_job_form_page_id', false ),
				get_option( 'easy_job_portal_job_dashboard_page_id', false ),
				get_option( 'easy_job_portal_jobs_page_id', false ),
			]
		);

		/**
		 * Filters a list of all page IDs related to DLJP.
		 *
		 * @since 1.30.0
		 *
		 * @param int[] $wpjm_page_ids
		 */
		$wpjm_page_ids = array_unique( apply_filters( 'easy_job_portal_page_ids', $wpjm_page_ids ) );
...
/**
 * Checks if the provided content or the current single page or post has a DLJP shortcode.
 *
 * @param string|null       $content   Content to check. If not provided, it uses the current post content.
 * @param string|array|null $tag Check specifically for one or more shortcodes. If not provided, checks for any DLJP shortcode.
 *
 * @return bool
 */
function has_wpjm_shortcode( $content = null, $tag = null ) {
	...
	if ( ! empty( $content ) ) {
		$wpjm_shortcodes = [ 'submit_job_form', 'job_dashboard', 'jobs', 'job_listing', 'job_summary', 'job_apply' ];
		/**
		 * Filters a list of all shortcodes associated with DLJP.
		 *
		 * @since 1.30.0
		 *
		 * @param string[] $wpjm_shortcodes
		 */
		$wpjm_shortcodes = array_unique( apply_filters( 'easy_job_portal_shortcodes', $wpjm_shortcodes ) );
...
function wpjm_use_standard_password_setup_email() {
	$use_standard_password_setup_email = 1 === intval( get_option( 'easy_job_portal_use_standard_password_setup_email' ) );
...
function wpjm_job_listing_employment_type_options() {
	$employment_types               = [];
	$employment_types['FULL_TIME']  = __( 'Full Time', 'easy-job-portal' );
	$employment_types['PART_TIME']  = __( 'Part Time', 'easy-job-portal' );
	$employment_types['CONTRACTOR'] = __( 'Contractor', 'easy-job-portal' );
	$employment_types['TEMPORARY']  = __( 'Temporary', 'easy-job-portal' );
	$employment_types['INTERN']     = __( 'Intern', 'easy-job-portal' );
	$employment_types['VOLUNTEER']  = __( 'Volunteer', 'easy-job-portal' );
	$employment_types['PER_DIEM']   = __( 'Per Diem', 'easy-job-portal' );
	$employment_types['OTHER']      = __( 'Other', 'easy-job-portal' );
...
function wpjm_job_listing_employment_type_enabled() {
	 ...
	 */
	return apply_filters( 'wpjm_job_listing_employment_type_enabled', (bool) get_option( 'easy_job_portal_enable_types' ) );
}
...
function wpjm_validate_new_password( $password ) {
	...
	/**
	 * Allows overriding default DLJP password validation rules.
	 *
	 * @since 1.27.0
	 *
...
function wpjm_get_password_rules_hint() {
	 ...
	 */
	return apply_filters( 'wpjm_password_rules_hint', __( 'Passwords must be at least 8 characters long.', 'easy-job-portal' ) );
}
 ...
 * @return bool
 */
function easy_job_portal_multi_job_type() {
	return apply_filters( 'easy_job_portal_multi_job_type', 1 === intval( get_option( 'easy_job_portal_multi_job_type' ) ) );
}
 ...
 * @return bool
 */
function easy_job_portal_enable_registration() {
	return apply_filters( 'easy_job_portal_enable_registration', 1 === intval( get_option( 'easy_job_portal_enable_registration' ) ) );
}
 ...
 * @return bool
 */
function easy_job_portal_generate_username_from_email() {
	return apply_filters( 'easy_job_portal_generate_username_from_email', 1 === intval( get_option( 'easy_job_portal_generate_username_from_email' ) ) );
}
 ...
 * @return bool
 */
function easy_job_portal_user_requires_account() {
	return apply_filters( 'easy_job_portal_user_requires_account', 1 === intval( get_option( 'easy_job_portal_user_requires_account' ) ) );
}
 ...
 * @return bool
 */
function easy_job_portal_user_can_edit_pending_submissions() {
	return apply_filters( 'easy_job_portal_user_can_edit_pending_submissions', 1 === intval( get_option( 'easy_job_portal_user_can_edit_pending_submissions' ) ) );
}
...
function wpjm_user_can_edit_published_submissions() {
	 ...
	 */
	return apply_filters( 'easy_job_portal_user_can_edit_published_submissions', in_array( get_option( 'easy_job_portal_user_edit_published_submissions' ), [ 'yes', 'yes_moderated' ], true ) );
}
...
function wpjm_published_submission_edits_require_moderation() {
	$require_moderation = 'yes_moderated' === get_option( 'easy_job_portal_user_edit_published_submissions' );
	 ...
	 */
	return apply_filters( 'easy_job_portal_published_submission_edits_require_moderation', $require_moderation );
}
 ...
 * @return string
 */
function easy_job_portal_dropdown_categories( $args = '' ) {
	$defaults = [
		'orderby'         => 'id',
		'order'           => 'ASC',
		'show_count'      => 0,
		...
		'name'            => 'cat',
		'id'              => '',
		'class'           => 'easy-job-portal-category-dropdown ' . ( is_rtl() ? 'chosen-rtl' : '' ),
		'depth'           => 0,
		'taxonomy'        => 'job_listing_category',
		'value'           => 'id',
		'multiple'        => true,
		'show_option_all' => false,
		'placeholder'     => __( 'Choose a category&hellip;', 'easy-job-portal' ),
		'no_results_text' => __( 'No results match', 'easy-job-portal' ),
		'multiple_text'   => __( 'Select Some Options', 'easy-job-portal' ),
	];
	...
	$categories_hash = 'jm_cats_' . md5( wp_json_encode( $r ) . Easy_Job_Portal_Cache_Helper::get_transient_version( 'jm_get_' . $r['taxonomy'] ) );
	...
	if ( ! empty( $categories ) ) {
		include_once EASY_JOB_PLUGIN_DIR . '/includes/class-easy-job-portal-category-walker.php';

		$walker = new Easy_Job_Portal_Category_Walker();
 ...
 * @return int
 */
function easy_job_portal_get_page_id( $page ) {
	$page_id = get_option( 'easy_job_portal_' . $page . '_page_id', false );
	if ( $page_id ) {
		/**
		 * Filters the page ID for a DLJP page.
		 *
		 * @since 1.26.0
		 *
		 * @param int $page_id
 ...
 * @return string|bool
 */
function easy_job_portal_get_permalink( $page ) {
	$page_id = easy_job_portal_get_page_id( $page );
	if ( $page_id ) {
...
/**
 * Filters the upload dir when $easy_job_portal_upload is true.
 *
 * @since 1.21.0
 * @param  array $pathdata
 * @return array
 */
function easy_job_portal_upload_dir( $pathdata ) {
	global $easy_job_portal_upload, $easy_job_portal_uploading_file;

	if ( ! empty( $easy_job_portal_upload ) ) {
		$dir = untrailingslashit( apply_filters( 'easy_job_portal_upload_dir', 'easy-job-portal-uploads/' . sanitize_key( $easy_job_portal_uploading_file ), sanitize_key( $easy_job_portal_uploading_file ) ) );
...
add_filter( 'upload_dir', 'easy_job_portal_upload_dir' );
 ...
 * @return array
 */
function easy_job_portal_prepare_uploaded_files( $file_data ) {
	$files_to_upload = [];
	...
	return apply_filters( 'easy_job_portal_prepare_uploaded_files', $files_to_upload );
}
 ...
 */
function easy_job_portal_upload_file( $file, $args = [] ) {
	global $easy_job_portal_upload, $easy_job_portal_uploading_file;
	...
	$easy_job_portal_upload         = true;
	$easy_job_portal_uploading_file = $args['file_key'];
	$uploaded_file              = new stdClass();
	if ( '' === $args['allowed_mime_types'] ) {
		$allowed_mime_types = easy_job_portal_get_allowed_mime_types( $easy_job_portal_uploading_file );
	} else {
	 ...
	 */
	$file = apply_filters( 'easy_job_portal_upload_file_pre_upload', $file, $args, $allowed_mime_types );
	...
	if ( ! in_array( $file['type'], $allowed_mime_types, true ) ) {
		...
		if ( $args['file_label'] ) {
			...
			return new WP_Error( 'upload', sprintf( __( '"%1$s" (filetype %2$s) needs to be one of the following file types: %3$s', 'easy-job-portal' ), $args['file_label'], $file['type'], $allowed_file_extensions ) );
		} else {
			// translators: %s is the list of allowed file types.
			return new WP_Error( 'upload', sprintf( __( 'Uploaded files need to be one of the following file types: %s', 'easy-job-portal' ), $allowed_file_extensions ) );
		}
	} else {
	...
	$easy_job_portal_upload         = false;
	$easy_job_portal_uploading_file = '';

	return $uploaded_file;
}

/**
 * Returns mime types specifically for DLJP.
 *
 * @since 1.25.1
 * @param   string $field Field used.
 * @return  array  Array of allowed mime types
 */
function easy_job_portal_get_allowed_mime_types( $field = '' ) {
	 ...
	 */
	return apply_filters( 'easy_job_portal_mime_types', $allowed_mime_types, $field );
}
...
function calculate_job_expiry( $job_id ) {
	...
	if ( ! $duration ) {
		$duration = absint( get_option( 'easy_job_portal_submission_duration' ) );
	}
 ...
 */
function easy_job_portal_duplicate_listing( $post_id ) {
	global $wpdb;
	...
	if ( ! empty( $post_meta ) ) {
		...
		$duplicate_ignore_keys         = apply_filters( 'easy_job_portal_duplicate_listing_ignore_keys', $default_duplicate_ignore_keys, true );
