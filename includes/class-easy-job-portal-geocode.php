<?php
/**
 * File containing the class Easy_Job_Portal_Geocode.
 *
...
class Easy_Job_Portal_Geocode {
	...
	public function __construct() {
		add_filter( 'easy_job_portal_geolocation_endpoint', [ $this, 'add_geolocation_endpoint_query_args' ], 0, 2 );
		add_filter( 'easy_job_portal_geolocation_api_key', [ $this, 'get_google_maps_api_key' ], 0 );
		add_action( 'easy_job_portal_update_job_data', [ $this, 'update_location_data' ], 20, 2 );
		add_action( 'easy_job_portal_job_location_edited', [ $this, 'change_location_data' ], 20, 2 );
	}
	...
	public function update_location_data( $job_id, $values ) {
		if ( apply_filters( 'easy_job_portal_geolocation_enabled', true ) && isset( $values['job_listing']['job_location'] ) ) {
			$address_data = self::get_location_data( $values['job_listing']['job_location'] );
	...
	public function change_location_data( $job_id, $new_location ) {
		if ( apply_filters( 'easy_job_portal_geolocation_enabled', true ) ) {
	...
	public function get_google_maps_api_key( $key ) {
		return get_option( 'easy_job_portal_google_maps_api_key' );
	}
	...
	public function add_geolocation_endpoint_query_args( $geocode_endpoint_url, $raw_address ) {
		...
		$api_key = apply_filters( 'easy_job_portal_geolocation_api_key', '', $raw_address );
		...
		$region = apply_filters( 'easy_job_portal_geolocation_region_cctld', '', $raw_address );
		if ( '' !== $region ) {
	...
	public static function get_location_data( $raw_address ) {
		...
		$geocode_api_url = apply_filters( 'easy_job_portal_geolocation_endpoint', self::GOOGLE_MAPS_GEOCODE_API_URL, $raw_address );
			...
			if ( false === $geocoded_address || empty( $geocoded_address->results[0] ) ) {
				$result           = wp_remote_get(
					...
					[
						...
						'redirection' => 1,
						'httpversion' => '1.1',
						'user-agent'  => 'WordPress/WP-Job-Manager-' . EASY_JOB_VERSION . '; ' . get_bloginfo( 'url' ),
						'sslverify'   => false,
					]
				);
				...
				if ( $geocoded_address->status ) {
					if ( 'ZERO_RESULTS' === $geocoded_address->status ) {
						throw new Exception( __( 'No results found', 'easy-job-portal' ) );
					} elseif ( 'OVER_QUERY_LIMIT' === $geocoded_address->status ) {
						set_transient( 'jm_geocode_over_query_limit', 1, HOUR_IN_SECONDS );
						throw new Exception( __( 'Query limit reached', 'easy-job-portal' ) );
						...
						throw new Exception( __( 'Geocoding error', 'easy-job-portal' ) );
					}
				} else {
					throw new Exception( __( 'Geocoding error', 'easy-job-portal' ) );
				}
		...
		return apply_filters( 'easy_job_portal_geolocation_get_location_data', $address, $geocoded_address );
	}
}

Easy_Job_Portal_Geocode::instance();
