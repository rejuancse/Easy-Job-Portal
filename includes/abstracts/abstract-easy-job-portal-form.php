<?php
/**
 * File containing the class Easy_Job_Portal_Form.
 *
 ...
 * @since 1.0.0
 */
abstract class Easy_Job_Portal_Form {
	...
	public function process() {
		...
		if (
			...
			isset( $_COOKIE['easy-job-portal-submitting-job-id'] ) &&
			isset( $_COOKIE['easy-job-portal-submitting-job-key'] ) &&
			get_post_meta( sanitize_text_field( wp_unslash( $_COOKIE['easy-job-portal-submitting-job-id'] ) ), '_submitting_key', true ) === $_COOKIE['easy-job-portal-submitting-job-key']
		) {
			delete_post_meta( sanitize_text_field( wp_unslash( $_COOKIE['easy-job-portal-submitting-job-id'] ) ), '_submitting_key' );
			setcookie( 'easy-job-portal-submitting-job-id', '', time() - 3600, COOKIEPATH, COOKIE_DOMAIN, false );
			setcookie( 'easy-job-portal-submitting-job-key', '', time() - 3600, COOKIEPATH, COOKIE_DOMAIN, false );
	...
	public function show_errors() {
		foreach ( $this->errors as $error ) {
			echo '<div class="easy-job-portal-error">' . wp_kses_post( $error ) . '</div>';
		}
	}
	...
	public function show_messages() {
		foreach ( $this->messages as $message ) {
			echo '<div class="easy-job-portal-info">' . wp_kses_post( $message ) . '</div>';
		}
	}
	...
	public function is_recaptcha_available() {
		$site_key               = get_option( 'easy_job_portal_recaptcha_site_key' );
		$secret_key             = get_option( 'easy_job_portal_recaptcha_secret_key' );
		 ...
		 */
		return apply_filters( 'easy_job_portal_is_recaptcha_available', $is_recaptcha_available );
	}
	...
	public function display_recaptcha_field() {
		$field             = [];
		$field['label']    = get_option( 'easy_job_portal_recaptcha_label' );
		$field['required'] = true;
		$field['site_key'] = get_option( 'easy_job_portal_recaptcha_site_key' );
	...
	public function validate_recaptcha_field( $success ) {
		...
		$recaptcha_field_label = get_option( 'easy_job_portal_recaptcha_label' );
		if ( empty( $input_recaptcha_response ) ) {
			// translators: Placeholder is for the label of the reCAPTCHA field.
			return new WP_Error( 'validation-error', sprintf( esc_html__( '"%s" check failed. Please try again.', 'easy-job-portal' ), $recaptcha_field_label ) );
		}
		...
		$response            = wp_remote_get(
			add_query_arg(
				[
					'secret'   => get_option( 'easy_job_portal_recaptcha_secret_key' ),
		...
		$validation_error = new WP_Error( 'validation-error', sprintf( esc_html__( '"%s" check failed. Please try again.', 'easy-job-portal' ), $recaptcha_field_label ) );
	...
	protected function get_posted_fields() {
		...
		foreach ( $this->fields as $group_key => $group_fields ) {
			foreach ( $group_fields as $key => $field ) {
				...
				$handler    = apply_filters( "easy_job_portal_get_posted_{$field_type}_field", false );
		 ...
		 */
		return apply_filters( 'easy_job_portal_get_posted_fields', $values, $this->fields );
	}
	...
	protected function get_posted_field( $key, $field ) {
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.NonceVerification -- Easy_Job_Portal_Form::sanitize_posted_field handles the sanitization based on the type of data passed; nonce check happens elsewhere.
	...
	protected function upload_file( $field_key, $field ) {
		if ( isset( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ]['name'] ) ) {
			if ( ! empty( $field['allowed_mime_types'] ) ) {
				...
				$allowed_mime_types = easy_job_portal_get_allowed_mime_types();
			}

			$file_urls       = [];
			$files_to_upload = easy_job_portal_prepare_uploaded_files( $_FILES[ $field_key ] ); //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- see https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1720.

			foreach ( $files_to_upload as $file_to_upload ) {
				$uploaded_file = easy_job_portal_upload_file(
					$file_to_upload,
					[
						'file_key'           => $field_key,
