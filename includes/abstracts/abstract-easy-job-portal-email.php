<?php
/**
 * File containing the class Easy_Job_Portal_Email.
 *
 ...
 * `easy_job_portal_email_init` hook. This will prevent unnecessary loading and won't include the files if this abstract
 * class isn't available.
 *
 * Example:
 * ```
 * add_action( 'easy_job_portal_email_init', 'custom_plugin_include_emails' );
 ...
 * @since 1.31.0
 */
abstract class Easy_Job_Portal_Email {
	/**
	...
	/**
	 * Easy_Job_Portal_Email constructor.
	 *
	...
	protected function prepare_args( $args ) {
		if ( isset( $args['job_id'] ) ) {
			...
			if ( $job instanceof WP_Post ) {
				$args['job_listing'] = $job;
			}
		}
		if ( isset( $args['job_listing'] ) && $args['job_listing'] instanceof WP_Post ) {
			$author = get_user_by( 'ID', $args['job_listing']->post_author );
