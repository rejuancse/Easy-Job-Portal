<?php
/**
 * File containing the class Easy_Job_Portal_Email_Employer_Expiring_Job.
 *
 ...
 */
class Easy_Job_Portal_Email_Employer_Expiring_Job extends Easy_Job_Portal_Email_Template {
	...
	public static function get_name() {
		return __( 'Employer Notice of Expiring Easy Job Portal', 'easy-job-portal' );
	}
	...
	public static function get_description() {
		return __( 'Send notices to employers before a job listing expires.', 'easy-job-portal' );
	}
	...
	public function get_subject() {
		...
		return sprintf( __( 'Job Listing Expiring: %s', 'easy-job-portal' ), $job->post_title );
	}
	...
	protected function prepare_args( $args ) {
		...
		if ( isset( $args['job_listing'] ) ) {
			$args['expiring_today'] = false;
			$today                  = date( 'Y-m-d', current_time( 'timestamp' ) );
			$expiring_date          = date( 'Y-m-d', strtotime( $args['job_listing']->_job_expires ) );
			if ( ! empty( $args['job_listing']->_job_expires ) && $today === $expiring_date ) {
	...
	public static function get_setting_fields() {
		...
		$fields[] = [
			...
			'label'      => __( 'Notice Period', 'easy-job-portal' ),
			'type'       => 'number',
			'after'      => ' ' . __( 'days', 'easy-job-portal' ),
			'attributes' => [ 'min' => 0 ],
		];
		return $fields;
	}
