<?php
/**
 * File containing the class Easy_Job_Portal_CPT.
 *
...
/**
 * Handles actions and filters specific to the custom post type for Easy Job Portal.
 *
 * @since 1.0.0
 */
class Easy_Job_Portal_CPT {
	...
	public function get_bulk_actions() {
		...
		$actions_handled['approve_jobs']         = [
			...
			'label'   => __( 'Approve %s', 'easy-job-portal' ),
			// translators: Placeholder (%s) is the plural name of the job listings post type.
			'notice'  => __( '%s approved', 'easy-job-portal' ),
			'handler' => [ $this, 'bulk_action_handle_approve_job' ],
		];
		$actions_handled['expire_jobs']          = [
			// translators: Placeholder (%s) is the plural name of the job listings post type.
			'label'   => __( 'Expire %s', 'easy-job-portal' ),
			// translators: Placeholder (%s) is the plural name of the job listings post type.
			'notice'  => __( '%s expired', 'easy-job-portal' ),
			'handler' => [ $this, 'bulk_action_handle_expire_job' ],
		];
		$actions_handled['mark_jobs_filled']     = [
			// translators: Placeholder (%s) is the plural name of the job listings post type.
			'label'   => __( 'Mark %s Filled', 'easy-job-portal' ),
			// translators: Placeholder (%s) is the plural name of the job listings post type.
			'notice'  => __( '%s marked as filled', 'easy-job-portal' ),
			'handler' => [ $this, 'bulk_action_handle_mark_job_filled' ],
		];
		$actions_handled['mark_jobs_not_filled'] = [
			// translators: Placeholder (%s) is the plural name of the job listings post type.
			'label'   => __( 'Mark %s Not Filled', 'easy-job-portal' ),
			// translators: Placeholder (%s) is the plural name of the job listings post type.
			'notice'  => __( '%s marked as not filled', 'easy-job-portal' ),
	...
	public function jobs_by_category() {
		...
		echo '<option value="" ' . selected( $selected_category, '', false ) . '>' . esc_html__( 'Select category', 'easy-job-portal' ) . '</option>';
	...
	public function jobs_meta_filters() {
		...
		$this->jobs_filter_dropdown(
			...
			[
				[
					'value' => '',
					'text'  => __( 'Select Filled', 'easy-job-portal' ),
				],
				[
					'value' => '1',
					'text'  => __( 'Filled', 'easy-job-portal' ),
				],
				[
					'value' => '0',
					'text'  => __( 'Not Filled', 'easy-job-portal' ),
				],
			]
		);
		...
		$this->jobs_filter_dropdown(
			...
			[
				[
					'value' => '',
					'text'  => __( 'Select Featured', 'easy-job-portal' ),
				],
				[
					'value' => '1',
					'text'  => __( 'Featured', 'easy-job-portal' ),
				],
				[
					'value' => '0',
					'text'  => __( 'Not Featured', 'easy-job-portal' ),
				],
			]
		);
	}
	...
	public function enter_title_here( $text, $post ) {
		if ( 'job_listing' === $post->post_type ) {
			return esc_html__( 'Position', 'easy-job-portal' );
		}
		return $text;
	}
	...
	public function post_updated_messages( $messages ) {
		...
		$messages['job_listing'] = [
			...
			1  => sprintf( __( '%1$s updated. <a href="%2$s">View</a>', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name, esc_url( get_permalink( $post_ID ) ) ),
			2  => __( 'Custom field updated.', 'easy-job-portal' ),
			3  => __( 'Custom field deleted.', 'easy-job-portal' ),
			// translators: %s is the singular name of the job listing post type.
			4  => sprintf( esc_html__( '%s updated.', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name ),
			// translators: %1$s is the singular name of the job listing post type; %2$s is the revision number.
			5  => $revision_title ? sprintf( __( '%1$s restored to revision from %2$s', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name, $revision_title ) : false,
			// translators: %1$s is the singular name of the job listing post type; %2$s is the URL to view the listing.
			6  => sprintf( __( '%1$s published. <a href="%2$s">View</a>', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name, esc_url( get_permalink( $post_ID ) ) ),
			// translators: %1$s is the singular name of the job listing post type; %2$s is the URL to view the listing.
			7  => sprintf( esc_html__( '%s saved.', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name ),
			// translators: %1$s is the singular name of the job listing post type; %2$s is the URL to preview the listing.
			8  => sprintf( __( '%1$s submitted. <a target="_blank" href="%2$s">Preview</a>', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name, esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
			9  => sprintf(
				...
				__( '%1$s scheduled for: <strong>%2$s</strong>. <a target="_blank" href="%3$s">Preview</a>', 'easy-job-portal' ),
			...
			// translators: %1$s is the singular name of the job listing post type; %2$s is the URL to view the listing.
			10 => sprintf( __( '%1$s draft updated. <a target="_blank" href="%2$s">Preview</a>', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name, esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
		];
	...
	/**
	 * Adds columns to admin listing of Easy Job Portal.
	 *
	 * @param array $columns
	 * @return array
	 */
	public function columns( $columns ) {
		...
		$columns['job_position']         = __( 'Position', 'easy-job-portal' );
		$columns['job_listing_type']     = __( 'Type', 'easy-job-portal' );
		$columns['job_location']         = __( 'Location', 'easy-job-portal' );
		$columns['job_status']           = '<span class="tips" data-tip="' . __( 'Status', 'easy-job-portal' ) . '">' . __( 'Status', 'easy-job-portal' ) . '</span>';
		$columns['job_posted']           = __( 'Posted', 'easy-job-portal' );
		$columns['job_expires']          = __( 'Expires', 'easy-job-portal' );
		$columns['job_listing_category'] = __( 'Categories', 'easy-job-portal' );
		$columns['featured_job']         = '<span class="tips" data-tip="' . __( 'Featured?', 'easy-job-portal' ) . '">' . __( 'Featured?', 'easy-job-portal' ) . '</span>';
		$columns['filled']               = '<span class="tips" data-tip="' . __( 'Filled?', 'easy-job-portal' ) . '">' . __( 'Filled?', 'easy-job-portal' ) . '</span>';
		$columns['job_actions']          = __( 'Actions', 'easy-job-portal' );
	...
	/**
	 * Displays the content for each custom column on the admin list for Easy Job Portal.
	 *
	 * @param mixed $column
	 */
	public function custom_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'job_position':
				...
				echo '<a href="' . esc_url( admin_url( 'post.php?post=' . $post->ID . '&action=edit' ) ) . '" class="tips job_title" data-tip="' . sprintf( esc_html__( 'ID: %d', 'easy-job-portal' ), intval( $post->ID ) ) . '">' . esc_html( wpjm_get_the_job_title() ) . '</a>';
			...
			case 'job_posted':
				...
				echo ( empty( $post->post_author ) ? esc_html__( 'by a guest', 'easy-job-portal' ) : sprintf( esc_html__( 'by %s', 'easy-job-portal' ), '<a href="' . esc_url( add_query_arg( 'author', $post->post_author ) ) . '">' . esc_html( get_the_author() ) . '</a>' ) ) . '</span>';
				break;
			case 'job_expires':
			...
			case 'job_actions':
				...
				if ( in_array( $post->post_status, [ 'pending', 'pending_payment' ], true ) && current_user_can( 'publish_post', $post->ID ) ) {
					$admin_actions['approve'] = [
						'action' => 'approve',
						'name'   => __( 'Approve', 'easy-job-portal' ),
				...
				if ( 'trash' !== $post->post_status ) {
					...
					if ( current_user_can( 'delete_post', $post->ID ) ) {
						$admin_actions['delete'] = [
							'action' => 'delete',
							'name'   => __( 'Delete', 'easy-job-portal' ),
							'url'    => get_delete_post_link( $post->ID ),
						];
					}
				}

				$admin_actions = apply_filters( 'easy_job_portal_admin_actions', $admin_actions, $post );
	...
	/**
	 * Filters the list table sortable columns for the admin list of Easy Job Portal.
	 *
	 * @param mixed $columns
	 * @return array
	 */
	...
	/**
	 * Sorts the admin listing of Easy Job Portal by updating the main query in the request.
	 *
