<?php
/**
 * File containing the class Easy_Job_Portal_Admin_Notices.
 *
...
/**
 * Easy_Job_Portal_Admin_Notices class.
 *
 * @since 1.32.0
 */
class Easy_Job_Portal_Admin_Notices {
	const STATE_OPTION      = 'easy_job_portal_admin_notices';
	...
	public static function init() {
		add_action( 'easy_job_portal_init_admin_notices', [ __CLASS__, 'init_core_notices' ] );
	...
	public static function init_core_notices() {
		// core_setup: Notice is used when first activating Easy Job Portal.
		add_action( 'easy_job_portal_admin_notice_' . self::NOTICE_CORE_SETUP, [ __CLASS__, 'display_core_setup' ] );
	}
	...
	public static function dismiss_notices() {
		if ( isset( $_GET['wpjm_hide_notice'] ) && isset( $_GET['_wpjm_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_wpjm_notice_nonce'] ), 'easy_job_portal_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce should not be modified.
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'easy-job-portal' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'You don&#8217;t have permission to do this.', 'easy-job-portal' ) );
			}
	...
	public static function display_notices() {
		/**
		 * Allows DLJP related plugins to set up their notice hooks.
		 *
		 * @since 1.32.0
		 */
		do_action( 'easy_job_portal_init_admin_notices' );
		...
		foreach ( $notice_state as $notice ) {
			...
			if ( ! apply_filters( 'easy_job_portal_show_admin_notice_' . $notice, true ) ) {
				continue;
			}
			 ...
			 * @since 1.32.0
			 */
			do_action( 'easy_job_portal_admin_notice_' . $notice );
		}
	}
	 ...
	 * @return bool
	 */
	public static function is_admin_on_standard_easy_job_portal_screen( $additional_screens = [] ) {
		...
		$show_on_screens = array_merge(
			[
				...
				'job_listing_page_easy-job-portal-addons',
				'job_listing_page_easy-job-portal-settings',
			],
			$additional_screens
		);
	...
	/**
	 * Displays the setup wizard notice when DLJP is first activated.
	 *
	 * Note: For internal use only. Do not call manually.
	 */
	public static function display_core_setup() {
		if ( ! self::is_admin_on_standard_easy_job_portal_screen( [ 'plugins', 'dashboard' ] ) ) {
			return;
		}
...
Easy_Job_Portal_Admin_Notices::init();
