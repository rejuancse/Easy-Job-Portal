<?php
/**
 * File containing the view for displaying the list of add-ons available to extend Easy Job Portal.
 *
...
echo '<h1 class="screen-reader-text">' . esc_html__( 'Easy Job Portal Add-ons', 'easy-job-portal' ) . '</h1>';
if ( ! empty( $messages ) ) {
	foreach ( $messages as $message ) {
		...
		$action_label  = isset( $message->action_label ) ? esc_attr( $message->action_label ) : __( 'More Information &rarr;', 'easy-job-portal' );
	...
	foreach ( $categories as $category ) {
		...
		<li>
			<a class="<?php echo $current_category === $category->slug ? 'current' : ''; ?>"
				href="<?php echo esc_url( admin_url( 'edit.php?post_type=job_listing&page=easy-job-portal-addons&category=' . esc_attr( $category->slug ) ) ); ?>">
...
if ( empty( $add_ons ) ) {
	echo '<div class="notice notice-warning below-h2"><p><strong>' . esc_html__( 'No add-ons were found.', 'easy-job-portal' ) . '</strong></p></div>';
} else {
