<?php
/**
 * File containing the class Easy_Job_Portal_Writepanels.
 *
 ...
 * @since 1.0.0
 */
class Easy_Job_Portal_Writepanels {
	...
	public function __construct() {
		...
		add_action( 'easy_job_portal_save_job_listing', [ $this, 'save_job_listing_data' ], 20, 2 );
	}
	...
	public function job_listing_fields() {
		...
		$fields_raw   = Easy_Job_Portal_Post_Types::get_job_listing_fields();
		$fields       = [];
		...
		if ( isset( $fields['_application'] ) && ! isset( $fields['_application']['default'] ) && 'url' !== get_option( 'easy_job_portal_allowed_application_method' ) ) {
		 ...
		 *
		 * To add job listing data fields, use the `easy_job_portal_job_listing_data_fields` found in `includes/class-easy-job-portal-post-types.php`.
		 *
		 * @since 1.33.0
		 *
		 * @param array    $fields  Job listing fields for WP admin. See `easy_job_portal_job_listing_data_fields` filter for more information.
		 * @param int|null $post_id Post ID to get fields for. May be null.
		 */
		$fields = apply_filters( 'easy_job_portal_job_listing_wp_admin_fields', $fields, $post_id );
	...
	public function add_meta_boxes() {
		...
		add_meta_box( 'job_listing_data', sprintf( __( '%s Data', 'easy-job-portal' ), $wp_post_types['job_listing']->labels->singular_name ), [ $this, 'job_listing_data' ], 'job_listing', 'normal', 'high' );
		...
		} elseif ( false === easy_job_portal_multi_job_type() ) {
			remove_meta_box( 'job_listing_typediv', 'job_listing', 'side' );
		...
		<p class="form-field">
			...
			if ( ! empty( $field['multiple'] ) ) {
				foreach ( (array) $field['value'] as $value ) {
					?>
					<span class="file_url"><input type="text" name="<?php echo esc_attr( $name ); ?>[]" placeholder="<?php echo esc_attr( $field['placeholder'] ); ?>" value="<?php echo esc_attr( $value ); ?>" /><button class="button button-small wp_easy_job_portal_upload_file_button" data-uploader_button_text="<?php esc_attr_e( 'Use file', 'easy-job-portal' ); ?>"><?php esc_html_e( 'Upload', 'easy-job-portal' ); ?></button><button class="button button-small wp_easy_job_portal_view_file_button"><?php esc_html_e( 'View', 'easy-job-portal' ); ?></button></span>
					<?php
				}
			} else {
				?>
			...
			if ( ! empty( $field['multiple'] ) ) {
				?>
				<button class="button button-small wp_easy_job_portal_add_another_file_button" data-field_name="<?php echo esc_attr( $key ); ?>" data-field_placeholder="<?php echo esc_attr( $field['placeholder'] ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Use file', 'easy-job-portal' ); ?>" data-uploader_button="<?php esc_attr_e( 'Upload', 'easy-job-portal' ); ?>" data-view_button="<?php esc_attr_e( 'View', 'easy-job-portal' ); ?>"><?php esc_html_e( 'Add file', 'easy-job-portal' ); ?></button>
				<?php
			}
			?>
		</p>
			...
			<span class="current-author">
				...
				if ( $posted_by ) {
					$user_string = sprintf(
						...
						esc_html__( '%1$s (#%2$s – %3$s)', 'easy-job-portal' ),
					...
					$user_string = __( 'Guest User', 'easy-job-portal' );
					echo esc_html( $user_string );
				}
				?>
				<a href="#" class="change-author button button-small"><?php esc_html_e( 'Change', 'easy-job-portal' ); ?></a>
			</span>
			<span class="hidden change-author">
				<select class="wpjm-user-search" id="easy_job_portal_user_search" name="<?php echo esc_attr( $name ); ?>" data-placeholder="<?php esc_attr_e( 'Guest', 'easy-job-portal' ); ?>" data-allow_clear="true">
	...
	/**
	 * Displays metadata fields for Easy Job Portal.
	 *
	 * @param int|WP_Post $post
	 */
	public function job_listing_data( $post ) {
		...
		echo '<div class="wp_easy_job_portal_meta_data">';

		wp_nonce_field( 'save_meta_data', 'easy_job_portal_nonce' );

		do_action( 'easy_job_portal_job_listing_data_start', $thepostid );
		...
		foreach ( $this->job_listing_fields() as $key => $field ) {
			...
			if ( has_action( 'easy_job_portal_input_' . $type ) ) {
				do_action( 'easy_job_portal_input_' . $type, $key, $field );
		...
		if ( $user_edited_date ) {
			...
			echo '<em>' . sprintf( esc_html__( '%1$s was last modified by the user on %2$s.', 'easy-job-portal' ), esc_html( $wp_post_types['job_listing']->labels->singular_name ), esc_html( date_i18n( get_option( 'date_format' ), $user_edited_date ) ) ) . '</em>';
			echo '</p>';
		}

		do_action( 'easy_job_portal_job_listing_data_end', $thepostid );
	...
	public function save_post( $post_id, $post ) {
		...
		if (
			empty( $_POST['easy_job_portal_nonce'] )
			|| ! wp_verify_nonce( wp_unslash( $_POST['easy_job_portal_nonce'] ), 'save_meta_data' ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce should not be modified.
		) {
			return;
		}
		...
		do_action( 'easy_job_portal_save_job_listing', $post_id, $post );
	}
	...
	public function save_job_listing_data( $post_id, $post ) {
		...
		foreach ( $this->job_listing_fields() as $key => $field ) {
			...
			if ( '_job_expires' === $key ) {
				// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce check handled by WP core.
				if ( empty( $_POST[ $key ] ) ) {
					if ( get_option( 'easy_job_portal_submission_duration' ) ) {
				...
				// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.NonceVerification.Missing -- Input sanitized in registered post meta config; see Easy_Job_Portal_Post_Types::register_meta_fields() and Easy_Job_Portal_Post_Types::get_job_listing_fields() methods.
		...
		if ( $is_job_listing_expired && ! $this->is_job_listing_status_changing( null, 'draft' ) ) {
			remove_action( 'easy_job_portal_save_job_listing', [ $this, 'save_job_listing_data' ], 20 );
...
Easy_Job_Portal_Writepanels::instance();
