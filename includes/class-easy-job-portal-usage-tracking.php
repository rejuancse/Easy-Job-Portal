<?php
/**
 * File containing the class Easy_Job_Portal_Usage_Tracking.
 *
...
/**
 * DLJP Usage Tracking subclass.
 */
class Easy_Job_Portal_Usage_Tracking extends Easy_Job_Portal_Usage_Tracking_Base {

	const DLJP_SETTING_NAME = 'easy_job_portal_usage_tracking_enabled';

	const DLJP_TRACKING_INFO_URL = '#';

	/**
	 * Easy_Job_Portal_Usage_Tracking constructor.
	 */
	protected function __construct() {
		parent::__construct();

		// Add filter for settings.
		add_filter( 'easy_job_portal_settings', [ $this, 'add_setting_field' ] );
	...
	protected function get_base_system_data() {
		$base_data            = [];
		$base_data['version'] = EASY_JOB_VERSION;

		return $base_data;
	}

	/**
	 * Track a Easy Job Portal event.
	 *
	 * @since 1.33.0
	 *
	...
	public static function log_event( $event_name, $properties = [] ) {
		$properties = array_merge(
			Easy_Job_Portal_Usage_Tracking_Data::get_event_logging_base_fields(),
			$properties
		);
	 ...
	 *
	 * @todo move this to Easy_Job_Portal_REST_API
	 * @return bool
	 */
	...
	public function get_tracking_enabled() {
		return get_option( self::DLJP_SETTING_NAME ) || false;
	}
	...
	public function set_tracking_enabled( $enable ) {
		update_option( self::DLJP_SETTING_NAME, $enable );
	}
	...
	protected function opt_in_dialog_text() {
		return sprintf(
			...
			__(
				'We\'d love if you helped us make Easy Job Portal better by allowing us to collect
				usage tracking data.',
				'easy-job-portal'
			),
			self::DLJP_TRACKING_INFO_URL
		);
	}
	...
	public function opt_in_checkbox_text() {
		return sprintf(
			 ...
			 * telling users what data DLJP tracks.
			 */
			__(
				'Help us make Easy Job Portal better by allowing us to collect
				usage tracking data.',
				'easy-job-portal'
			),
			self::DLJP_TRACKING_INFO_URL
	...
	public function add_setting_field( $fields ) {
		$fields['general'][1][] = [
			'name'     => self::DLJP_SETTING_NAME,
			'std'      => '0',
			'type'     => 'checkbox',
			'desc'     => '',
			'label'    => __( 'Enable Usage Tracking', 'easy-job-portal' ),
	...
	public function clear_options() {
		delete_option( self::DLJP_SETTING_NAME );
