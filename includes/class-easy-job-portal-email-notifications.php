<?php
/**
 * File containing the class Easy_Job_Portal_Email_Notifications.
 *
...
/**
 * Base class for Easy Job Portal's email notification system.
 *
 * @since 1.31.0
 */
final class Easy_Job_Portal_Email_Notifications {
	const EMAIL_SETTING_PREFIX     = 'easy_job_portal_email_';
	...
	public static function init() {
		add_action( 'easy_job_portal_send_notification', [ __CLASS__, 'schedule_notification' ], 10, 2 );
		add_action( 'easy_job_portal_email_init', [ __CLASS__, 'lazy_init' ] );
		add_action( 'easy_job_portal_email_job_details', [ __CLASS__, 'output_job_details' ], 10, 4 );
		add_action( 'easy_job_portal_email_header', [ __CLASS__, 'output_header' ], 10, 3 );
		add_action( 'easy_job_portal_email_footer', [ __CLASS__, 'output_footer' ], 10, 3 );
		add_action( 'easy_job_portal_email_daily_notices', [ __CLASS__, 'send_employer_expiring_notice' ] );
		add_action( 'easy_job_portal_email_daily_notices', [ __CLASS__, 'send_admin_expiring_notice' ] );
		add_filter( 'easy_job_portal_settings', [ __CLASS__, 'add_easy_job_portal_email_settings' ], 1 );
		add_action( 'easy_job_portal_job_submitted', [ __CLASS__, 'send_new_job_notification' ] );
		add_action( 'easy_job_portal_user_edit_job_listing', [ __CLASS__, 'send_updated_job_notification' ] );
	}

	/**
	 * Gets list of email notifications handled by Easy Job Portal core.
	 *
	 * @return array
	 */
	...
	public static function maybe_init() {
		if ( 0 === did_action( 'easy_job_portal_email_init' ) ) {
			/**
			 ...
			 * @since 1.31.0
			 */
			do_action( 'easy_job_portal_email_init' );
		}
	}
	...
	public static function lazy_init() {
		...
		include_once EASY_JOB_PLUGIN_DIR . '/includes/emails/class-easy-job-portal-email-admin-new-job.php';
		include_once EASY_JOB_PLUGIN_DIR . '/includes/emails/class-easy-job-portal-email-admin-updated-job.php';
		include_once EASY_JOB_PLUGIN_DIR . '/includes/emails/class-easy-job-portal-email-employer-expiring-job.php';
		include_once EASY_JOB_PLUGIN_DIR . '/includes/emails/class-easy-job-portal-email-admin-expiring-job.php';

		if ( ! class_exists( 'Emogrifier' ) && class_exists( 'DOMDocument' ) && version_compare( PHP_VERSION, '5.5', '>=' ) ) {
			include_once EASY_JOB_PLUGIN_DIR . '/lib/emogrifier/class-emogrifier.php';
		}
	}
	...
	public static function clear_deferred_notifications() {
		if ( ! defined( 'PHPUNIT_DLJP_TESTSUITE' ) || ! PHPUNIT_DLJP_TESTSUITE ) {
	...
	/**
	 * Gets a list of all email notifications that Easy Job Portal handles.
	 *
	...
	public static function get_email_notifications( $enabled_notifications_only = false ) {
		 ...
		 */
		$email_notification_classes = array_unique( apply_filters( 'easy_job_portal_email_notifications', self::core_email_notifications() ) );
		$email_notifications        = [];

		/**
		 * Email class in loop.
		 *
		 * @var Easy_Job_Portal_Email $email_class
		 */
	 ...
	 * @param Easy_Job_Portal_Email $email          Email object for the notification.
	...
	private static function get_job_detail_fields( WP_Post $job, $sent_to_admin, $plain_text = false ) {
		...
		if ( get_option( 'easy_job_portal_enable_types' ) && wp_count_terms( 'job_listing_type' ) > 0 ) {
		...
		if ( get_option( 'easy_job_portal_enable_categories' ) && wp_count_terms( 'job_listing_category' ) > 0 ) {
		...
		if ( ! empty( $job_expires ) ) {
			...
			$fields['job_expires'] = [
				'label' => __( 'Listing expires', 'easy-job-portal' ),
		...
		if ( $sent_to_admin ) {
			...
			if ( $author instanceof WP_User ) {
				$fields['author'] = [
					'label' => __( 'Posted by', 'easy-job-portal' ),
		 ...
		 */
		return apply_filters( 'easy_job_portal_emails_job_detail_fields', $fields, $job, $sent_to_admin, $plain_text );
	}
	 ...
	 * Useful to provide alternative email headers and footers for a specific DLJP extension plugin.
	 *
	 * @param string $email_notification_key  Email notification key for email being sent.
	...
	private static function email_template_path_alternative( $email_notification_key, $template_name, $plain_text ) {
		...
		if ( ! $email_class || ! is_subclass_of( $email_class, 'Easy_Job_Portal_Email_Template' ) ) {
			return false;
		}
	 ...
	 * @return string
	 */
	public static function locate_template_file( $template_name, $plain_text = false, $template_path = 'easy_job_portal', $default_path = '' ) {
		return locate_easy_job_portal_template( Easy_Job_Portal_Email_Template::generate_template_file_name( $template_name, $plain_text ), $template_path, $default_path );
	}
	 ...
	 * @return array
	 */
	public static function add_easy_job_portal_email_settings( $settings ) {
		return self::add_email_settings( $settings, Easy_Job_Portal_Email::get_context() );
	}
	...
	public static function add_email_settings( $settings, $context ) {
		...
		if ( ! empty( $email_settings ) ) {
			$settings['email_notifications'] = [
				__( 'Email Notifications', 'easy-job-portal' ),
				$email_settings,
				[
					'before' => __( 'Select the email notifications to enable.', 'easy-job-portal' ),
				],
			];
		}
	...
	public static function is_email_notification_enabled( $email_notification_key ) {
		 ...
		 * @param string $email_notification_key
		 */
		return apply_filters( 'easy_job_portal_email_is_email_notification_enabled', $is_email_notification_enabled, $email_notification_key );
	}
	...
	public static function send_as_plain_text( $email_notification_key, $args = [] ) {
		 ...
		 * @param array  $args                    Email arguments.
		 */
		return apply_filters( 'easy_job_portal_email_send_as_plain_text', $send_as_plain_text, $email_notification_key, $args );
	}
	...
	public static function send_employer_expiring_notice() {
		...
		$email_key = Easy_Job_Portal_Email_Employer_Expiring_Job::get_key();
		if ( ! self::is_email_notification_enabled( $email_key ) ) {
			return;
		}
		$settings    = self::get_email_settings( $email_key );
		$days_notice = Easy_Job_Portal_Email_Employer_Expiring_Job::get_notice_period( $settings );
	...
	public static function send_admin_expiring_notice() {
		...
		$email_key = Easy_Job_Portal_Email_Admin_Expiring_Job::get_key();
		if ( ! self::is_email_notification_enabled( $email_key ) ) {
			return;
		}
		$settings    = self::get_email_settings( $email_key );
		$days_notice = Easy_Job_Portal_Email_Admin_Expiring_Job::get_notice_period( $settings );
		self::send_expiring_notice( $email_key, $days_notice );
	}
	...
	public static function send_new_job_notification( $job_id ) {
		do_action( 'easy_job_portal_send_notification', 'admin_new_job', [ 'job_id' => $job_id ] );
	}
	...
	public static function send_updated_job_notification( $job_id ) {
		do_action( 'easy_job_portal_send_notification', 'admin_updated_job', [ 'job_id' => $job_id ] );
	}
	...
	private static function send_expiring_notice( $email_notification_key, $days_notice ) {
		...
		if ( $job_ids ) {
			foreach ( $job_ids as $job_id ) {
				do_action( 'easy_job_portal_send_notification', $email_notification_key, [ 'job_id' => $job_id ] );
			}
		}
	}
	...
	private static function get_email_setting_fields( $email_notification_key ) {
		...
		$core_settings  = [
			[
				'name'    => 'plain_text',
				'std'     => '0',
				'label'   => __( 'Format', 'easy-job-portal' ),
				'type'    => 'radio',
				'options' => [
					'1' => __( 'Send plain text email', 'easy-job-portal' ),
					'0' => __( 'Send rich text email', 'easy-job-portal' ),
				],
			],
		];
	...
	private static function is_email_notification_valid( $email_class ) {
		...
		return is_string( $email_class )
				...
				&& is_subclass_of( $email_class, 'Easy_Job_Portal_Email' )
	 ...
	 *
	 * @param string               $email_notification_key
	 * @param Easy_Job_Portal_Email $email
	 * @return bool
	 */
	private static function send_email( $email_notification_key, Easy_Job_Portal_Email $email ) {
		if ( ! $email->is_valid() ) {
			return false;
		}
		...
		foreach ( $fields as $field ) {
			 ...
			 * @param Easy_Job_Portal_Email $email             Email notification object.
			 */
			$args[ $field ] = apply_filters( "easy_job_portal_email_{$email_notification_key}_{$field}", $email->$method(), $email );
		}
		...
		foreach ( $send_to as $to_email ) {
			 ...
			 * @param Easy_Job_Portal_Email $email             Email notification object.
			 */
			$args = apply_filters( "easy_job_portal_email_{$email_notification_key}_args", $args, $email );
			 ...
			 * @param
			 */
			if ( ! apply_filters( 'easy_job_portal_email_do_send_notification', true, $email, $args, $content, $headers ) ) {
				continue;
			}
	...
	private static function get_email_content( $email_notification_key, $args ) {
		 ...
		 */
		do_action( 'easy_job_portal_email_header', $email_notification_key, $args, $plain_text );
		 ...
		 * @param bool   $plain_text             True if sending plain text email.
		 */
		do_action( 'easy_job_portal_email_footer', $email_notification_key, $args, $plain_text );
		 ...
		 * @param bool   $plain_text             True if sending plain text email.
		 */
		return apply_filters( 'easy_job_portal_email_content', $content, $email_notification_key, $args, $plain_text );
	}
