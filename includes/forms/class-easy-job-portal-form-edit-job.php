w<?php
/**
 * File containing the class Easy_Job_Portal_Form_Edit_Job.
 *
...
require_once 'class-easy-job-portal-form-submit-job.php';

/**
 * Handles the editing of Easy Job Portal from the public facing frontend (from within `[job_dashboard]` shortcode).
 *
 * @since 1.0.0
 * @extends Easy_Job_Portal_Form_Submit_Job
 */
class Easy_Job_Portal_Form_Edit_Job extends Easy_Job_Portal_Form_Submit_Job {
	 ...
	 *
	 * @access protected
	 * @var Easy_Job_Portal_Form_Edit_Job The single instance of the class
	 */
	...
	public function __construct() {
		...
		if ( ! empty( $this->job_id ) ) {
			if ( ! Easy_Job_Portal_Post_Types::job_is_editable( $this->job_id ) ) {
				$this->job_id = 0;
			}
		}
	}
	...
	public function output( $atts = [] ) {
		if ( ! empty( $this->save_message ) ) {
			echo '<div class="easy-job-portal-message">' . wp_kses_post( $this->save_message ) . '</div>';
		}
		if ( ! empty( $this->save_error ) ) {
			echo '<div class="easy-job-portal-error">' . wp_kses_post( $this->save_error ) . '</div>';
		}
		$this->submit();
	}
	...
	public function submit() {
		...
		if ( empty( $this->job_id ) ) {
			echo wp_kses_post( wpautop( __( 'Invalid listing', 'easy-job-portal' ) ) );
			return;
		}
		...
		$save_button_text = __( 'Save changes', 'easy-job-portal' );
		if (
			'publish' === get_post_status( $this->job_id )
			&& wpjm_published_submission_edits_require_moderation()
		) {
			$save_button_text = __( 'Submit changes for approval', 'easy-job-portal' );
		}
	...
	public function submit_handler() {
		...
		try {
			...
			// Update the job.
			$this->save_job( $values['job_listing']['job_title'], $values['job_listing']['job_description'], $save_post_status, $values, false );
			$this->update_job_data( $values );

			// Successful.
			$save_message = __( 'Your changes have been saved.', 'easy-job-portal' );
			$post_status  = get_post_status( $this->job_id );

			update_post_meta( $this->job_id, '_job_edited', time() );

			if ( 'publish' === $post_status ) {
				$save_message = $save_message . ' <a href="' . get_permalink( $this->job_id ) . '">' . __( 'View &rarr;', 'easy-job-portal' ) . '</a>';
			} elseif ( 'publish' === $original_post_status && 'pending' === $post_status ) {
				$save_message = __( 'Your changes have been submitted and your listing will be visible again once approved.', 'easy-job-portal' );
				 ...
				 */
				if ( apply_filters( 'easy_job_portal_reset_listing_expiration_on_user_edit', false ) ) {
			 ...
			 */
			do_action( 'easy_job_portal_user_edit_job_listing', $this->job_id, $save_message, $values );
			 ...
			 * @param array  $values        Submitted values for job listing.
			 */
			$this->save_message = apply_filters( 'easy_job_portal_update_job_listings_message', $save_message, $this->job_id, $values );
