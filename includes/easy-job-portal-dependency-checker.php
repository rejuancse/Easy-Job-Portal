<?php
/**
 * File containing the class Easy_Job_Portal_Dependency_Checker.
 *
...
/**
 * Handles checking for Easy Job Portal's dependencies.
 *
 * @since 1.33.0
 */
class Easy_Job_Portal_Dependency_Checker {
	const MINIMUM_PHP_VERSION = '5.6.20';
	const MINIMUM_WP_VERSION  = '4.9.0';

	/**
	 * Check if Easy Job Portal's dependencies have been met.
	 *
	 * @return bool True if we should continue to load the plugin.
	 */
	public static function check_dependencies() {
		if ( ! self::check_php() ) {
			add_action( 'admin_notices', array( 'Easy_Job_Portal_Dependency_Checker', 'add_php_notice' ) );
		...
		if ( ! self::check_wp() ) {
			add_action( 'admin_notices', array( 'Easy_Job_Portal_Dependency_Checker', 'add_wp_notice' ) );
			add_filter( 'plugin_action_links_' . EASY_JOB_PLUGIN_BASENAME, array( 'Easy_Job_Portal_Dependency_Checker', 'wp_version_plugin_action_notice' ) );
		}
	...
	public static function add_php_notice() {
		...
		// translators: %1$s is version of PHP that Easy Job Portal requires; %2$s is the version of PHP WordPress is running on.
		$message = sprintf( __( '<strong>Easy Job Portal</strong> requires a minimum PHP version of %1$s, but you are running %2$s.', 'easy-job-portal' ), self::MINIMUM_PHP_VERSION, phpversion() );
		...
		printf(
			...
			esc_html__( 'Learn more about updating PHP', 'easy-job-portal' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'easy-job-portal' )
		);
		echo '</p></div>';
	}
	...
	public static function deactivate_self() {
		deactivate_plugins( EASY_JOB_PLUGIN_BASENAME );
	}
	...
	public static function add_wp_notice() {
		...
		echo '<p>' . wp_kses_post( __( '<strong>Easy Job Portal</strong> requires a more recent version of WordPress.', 'easy-job-portal' ) . $update_action_link ) . '</p>';
		echo '</div>';
	}
	...
	public static function wp_version_plugin_action_notice( $actions ) {
		if ( ! current_user_can( 'update_core' ) ) {
			$actions[] = '<strong style="color: red">' . esc_html__( 'WordPress Update Required', 'easy-job-portal' ) . '</strong>';
		} else {
			$actions[] = '<a href="' . esc_url( self_admin_url( 'update-core.php' ) ) . '" style="color: red">' . esc_html__( 'WordPress Update Required', 'easy-job-portal' ) . '</a>';
		}
		return $actions;
	}
	...
	private static function get_critical_screen_ids() {
		return array( 'dashboard', 'plugins', 'plugins-network', 'edit-job_listing', 'job_listing_page_easy-job-portal-settings' );
	}
}
