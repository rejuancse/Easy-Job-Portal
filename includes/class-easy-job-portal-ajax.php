<?php
/**
 * File containing the class Easy_Job_Portal_Ajax.
 *
...
class Easy_Job_Portal_Ajax {
	...
	public function __construct() {
		...
		// JM Ajax endpoints.
		add_action( 'easy_job_portal_ajax_get_listings', [ $this, 'get_listings' ] );
		add_action( 'easy_job_portal_ajax_upload_file', [ $this, 'upload_file' ] );

		// BW compatible handlers.
		add_action( 'wp_ajax_nopriv_easy_job_portal_get_listings', [ $this, 'get_listings' ] );
		add_action( 'wp_ajax_easy_job_portal_get_listings', [ $this, 'get_listings' ] );
		add_action( 'wp_ajax_nopriv_easy_job_portal_upload_file', [ $this, 'upload_file' ] );
		add_action( 'wp_ajax_easy_job_portal_upload_file', [ $this, 'upload_file' ] );
		add_action( 'wp_ajax_easy_job_portal_search_users', [ $this, 'ajax_search_users' ] );
	}
	...
	public static function do_jm_ajax() {
		...
		if ( $action ) {
			 ...
			 * @since 1.23.0
			 */
			do_action( 'easy_job_portal_ajax_' . sanitize_text_field( $action ) );
			wp_die();
		}
	}

	/**
	 * Returns Easy Job Portal for Ajax endpoint.
	 */
	public function get_listings() {
		...
		$per_page           = isset( $_REQUEST['per_page'] ) ? absint( $_REQUEST['per_page'] ) : absint( get_option( 'easy_job_portal_per_page' ) );
		 ...
		 */
		$jobs = get_job_listings( apply_filters( 'easy_job_portal_get_listings_args', $args ) );
		...
		if ( $jobs->post_count && ( $search_location || $search_keywords || $search_categories || $job_types_filtered ) ) {
			...
			$message               = sprintf( _n( 'Search completed. Found %d matching record.', 'Search completed. Found %d matching records.', $jobs->found_posts, 'easy-job-portal' ), $jobs->found_posts );
		 ...
		 * }
		 */
		$result['showing'] = apply_filters( 'easy_job_portal_get_listings_custom_filter_text', $message, $search_values );

		// Generate RSS link.
		$result['showing_links'] = easy_job_portal_get_filtered_links(
			[
		 ...
		 */
		if ( true !== apply_filters( 'easy_job_portal_ajax_get_jobs_html_results', true, $result, $jobs ) ) {
			/**
			 ...
			 *                              `easy_job_portal_ajax_get_jobs_html_results` returns true).
			 *  @type array $pagination     Pagination links to use for stepping through filter results.
			 * }
			 */
			wp_send_json( apply_filters( 'easy_job_portal_get_listings_result', $result, $jobs ) );
		...
		if ( $result['found_jobs'] ) {
			...
			get_easy_job_portal_template_part( 'content', 'no-jobs-found' );
		}
		...
		/** This filter is documented in includes/class-easy-job-portal-ajax.php (above) */
		wp_send_json( apply_filters( 'easy_job_portal_get_listings_result', $result, $jobs ) );
	}
	...
	public function upload_file() {
		if ( ! easy_job_portal_user_can_upload_file_via_ajax() ) {
			wp_send_json_error( __( 'You must be logged in to upload files using this method.', 'easy-job-portal' ) );
			return;
		}
		$data = [
			'files' => [],
		...
		if ( ! empty( $_FILES ) ) {
			foreach ( $_FILES as $file_key => $file ) {
				$files_to_upload = easy_job_portal_prepare_uploaded_files( $file );
	...
	private static function user_can_search_users() {
		 ...
		 */
		$allowed_capabilities = apply_filters( 'easy_job_portal_caps_can_search_users', [ 'edit_job_listings' ] );
		 ...
		 */
		return apply_filters( 'easy_job_portal_user_can_search_users', $user_can_search_users );
	}
	...
	public static function ajax_search_users() {
		...
		if ( empty( $users ) ) {
			 ...
			 */
			$search_args = apply_filters( 'easy_job_portal_search_users_args', $search_args, $term, $exclude, $page );
		 ...
		 * @param int    $page        Current page.
		 */
		$response = apply_filters( 'easy_job_portal_search_users_response', $response, $term, $exclude, $page );

		wp_send_json( $response );
	}
}

Easy_Job_Portal_Ajax::instance();
