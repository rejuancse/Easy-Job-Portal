...
<div class="job_application application">
    <?php if ( $employee ) :
		...
		<?php
			printf(
				esc_html__( 'Already applied on %s', DJP_DOMAIN ),
				date_i18n( 'd M, Y', strtotime( $application->created_at ) )
			); 
		?>
		</strong>
		<?php else: ?>
			<button data-id="<?php echo esc_attr($post_id); ?>" data-message="<?php esc_attr_e( 'Are you sure to apply to this job?', DJP_DOMAIN ); ?>" id="wljp-job-apply-button">
				<?php esc_html_e( 'Apply Now', DJP_DOMAIN ); ?>
			</button>
		<?php endif; ?>
		<?php else : ?>
			<a href="<?php echo esc_url($account_page_url); ?>" class="application_button button"><?php esc_html_e( 'Apply for job', DJP_DOMAIN ); ?></a>	
	<?php endif; ?>
</div>
