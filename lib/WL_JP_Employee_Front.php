...
class WL_JP_Employee_Front {
	...
	public static function register_cv() {
		...
		if ( empty( $personal_name ) ) {
			$errors['employee_personal_name'] = esc_html__( 'Please specify your name', EJP_DOMAIN );
		}
		...
		if ( ! empty( $document_cv ) ) {
			...
			if ( ! in_array( $file_type, $allowed_file_types ) ) {
				$errors['employee_document_cv'] = esc_html__( 'Please provide CV in PDF, DOC or DOCX format.', EJP_DOMAIN );
			}
		}
