...
*
*/
function get_easy_job_portal_template( $template_name, $args = [], $template_path = 'easy_job_portal', $default_path = '' ) {
	if ( $args && is_array( $args ) ) {
		// phpcs:ignore WordPress.PHP.DontExtract.extract_extract -- Please, forgive us.
		extract( $args );
	}
	include locate_easy_job_portal_template( $template_name, $template_path, $default_path );
}

function locate_easy_job_portal_template( $template_name, $template_path = 'easy_job_portal', $default_path = '' ) {
	...
	if ( ! $template && false !== $default_path ) {
		$default_path = $default_path ? $default_path : EASY_JOB_PLUGIN_DIR . '/templates/';
	...
	return apply_filters( 'easy_job_portal_locate_template', $template, $template_name, $template_path );
}

function get_easy_job_portal_template_part( $slug, $name = '', $template_path = 'easy_job_portal', $default_path = '' ) {
	$template = '';

	if ( $name ) {
		$template = locate_easy_job_portal_template( "{$slug}-{$name}.php", $template_path, $default_path );
	}

	// If template file doesn't exist, look in yourtheme/slug.php and yourtheme/easy_job_portal/slug.php.
	if ( ! $template ) {
		$template = locate_easy_job_portal_template( "{$slug}.php", $template_path, $default_path );
	}
...
function easy_job_portal_body_class( $classes ) {
...
add_filter( 'body_class', 'easy_job_portal_body_class' );
...
function get_the_job_status( $post = null ) {
	...
	if ( isset( $statuses[ $status ] ) ) {
		...
		$status = esc_html__( 'Inactive', 'easy-job-portal' );
	}
...
function employees_can_apply( $post = null ) {
	$post = get_post( $post );
	return apply_filters( 'easy_job_portal_employees_can_apply', ( ! is_position_filled() && ! in_array( $post->post_status, [ 'preview', 'expired' ], true ) ), $post );
}
...
function get_the_job_application_method( $post = null ) {
	...
	if ( strstr( $apply, '@' ) && is_email( $apply ) ) {
		...
		$method->subject = apply_filters( 'easy_job_portal_application_email_subject', sprintf( esc_html__( 'Application via %1$s listing on %2$s', 'easy-job-portal' ), esc_html( $post->post_title ), esc_url( home_url() ) ), $post );
	} else {
...
function wpjm_the_job_description( $post = null ) {
	...
	if ( $job_description ) {
		Easy_Job_Portal_Post_Types::output_kses_post( $job_description );
	}
}
...
function wpjm_get_the_job_types( $post = null ) {
	...
	if ( ! empty( $types ) && ! easy_job_portal_multi_job_type() ) {
		$types = [ current( $types ) ];
	}
...
function wpjm_get_registration_fields() {
	$generate_username_from_email      = easy_job_portal_generate_username_from_email();
	...
	if ( easy_job_portal_enable_registration() ) {
		...
		$registration_fields['create_account_email'] = [
			'type'        => 'text',
			'label'       => esc_html__( 'Your email', 'easy-job-portal' ),
			'placeholder' => __( 'you@yourdomain.com', 'easy-job-portal' ),
		...
		$registration_fields['create_account_username'] = [
			'type'     => 'text',
			'label'    => esc_html__( 'Username', 'easy-job-portal' ),
			'required' => $account_required,
		...
		$registration_fields['create_account_password'] = [
			'type'         => 'password',
			'label'        => esc_html__( 'Password', 'easy-job-portal' ),
			'autocomplete' => false,
			'required'     => $account_required,
		];
		...
		$registration_fields['create_account_password_verify'] = [
			'type'         => 'password',
			'label'        => esc_html__( 'Verify Password', 'easy-job-portal' ),
			'autocomplete' => false,
			'required'     => $account_required,
		];
...
function the_job_publish_date( $post = null ) {
	$date_format = get_option( 'easy_job_portal_date_format' );

	if ( 'default' === $date_format ) {
		$display_date = esc_html__( 'Posted on ', 'easy-job-portal' ) . date_i18n( get_option( 'date_format' ), get_post_time( 'U' ) );
	} else {
		// translators: Placeholder %s is the relative, human readable time since the job listing was posted.
		$display_date = sprintf( esc_html__( 'Posted %s ago', 'easy-job-portal' ), human_time_diff( get_post_time( 'U' ), current_time( 'timestamp' ) ) );
	}
...
function get_the_job_publish_date( $post = null ) {
	...
	if ( 'default' === $date_format ) {
		...
		// translators: Placeholder %s is the relative, human readable time since the job listing was posted.
		return sprintf( __( 'Posted %s ago', 'easy-job-portal' ), human_time_diff( get_post_time( 'U' ), current_time( 'timestamp' ) ) );
	}
}
...
function the_job_location( $map_link = true, $post = null ) {
	...
	if ( $location ) {
		...
		echo wp_kses_post( apply_filters( 'the_job_location_anywhere_text', __( 'Anywhere', 'easy-job-portal' ) ) );
	}
}
...
function the_company_logo( $size = 'thumbnail', $default = null, $post = null ) {
	...
	if ( has_post_thumbnail( $post ) ) {
		...
		if ( 'full' !== $size ) {
			$logo = easy_job_portal_get_resized_image( $logo, $size );
		}
		echo '<img class="company_logo" src="' . esc_url( $logo ) . '" alt="' . esc_attr( get_the_company_name( $post ) ) . '" />';
	} elseif ( $default ) {
		echo '<img class="company_logo" src="' . esc_url( $default ) . '" alt="' . esc_attr( get_the_company_name( $post ) ) . '" />';
	} else {
		echo '<img class="company_logo" src="' . esc_url( apply_filters( 'easy_job_portal_default_company_logo', EASY_JOB_PLUGIN_URL . '/assets/images/company.png' ) ) . '" alt="' . esc_attr( get_the_company_name( $post ) ) . '" />';
	}
}
 ...
 * @return string
 */
function easy_job_portal_get_resized_image( $logo, $size ) {
...
function job_listing_meta_display() {
	get_easy_job_portal_template( 'content-single-job_listing-meta.php', [] );
}
...
function job_listing_company_display() {
	get_easy_job_portal_template( 'content-single-job_listing-company.php', [] );
}
